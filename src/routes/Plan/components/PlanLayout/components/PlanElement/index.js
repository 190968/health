...
const PlanElementPure = (props) => {
    //console.log(props);
    const {i,element, date, isDraggable, onDrop, isBuilderMode, isPreviewMode, plan, upid, schedule, mode, notClickable=false } = props;
    ...
    return (
        <React.Fragment>
            ...
            {(props.showChildren) && <div style={{padding:20, paddingRight:0, background: '#fbfbfb', border: '1px solid #e8e8e8',
                borderTop:'none'}} ><PlanElementChildrenList elementId={id} onDrop={onDrop} plan={plan} isDraggable={isDraggable} isPreviewMode={isPreviewMode} isBuilderMode={isBuilderMode} mode={mode} elementValue={props.elementValue} notClickable={notClickable}  /></div>}
        </React.Fragment>
        )
}
