...
import MedicationManager from './Medication/containers/MedicationManager';
	...
	render() {
		...
		const menu = (
			<Menu onClick={this.handleMenuClick}>
				...
				<Menu.Item disabled key="commitment">
					Make a Commitment
				</Menu.Item>
				<Menu.Item disabled key="promise">
					Make a Promise
				</Menu.Item>
				<Menu.Item disabled key="print">
					Print
				</Menu.Item>
			</Menu>
		);
		return (
			<Card
				...
				extra={
					<div>
						<Button.Group>
							<Tooltip title={<FormattedMessage id="plan.prev_day" defaultMessage="Previous day" />}>
								<Button size="small" onClick={() => this.showDate('prev')}>
									<Icon type="left" />
								</Button>
							</Tooltip>
							<Tooltip title={<FormattedMessage id="plan.next_day" defaultMessage="Next day" />}>
						...
						<Button.Group style={{ marginLeft: 10 }}>
							...
							<Tooltip
								title={<FormattedMessage id="medication.add" defaultMessage="Add Medication" />}
								placement={'top'}
							>
								<Popover
									content={<MedicationSelect userId={user_id} onSelect={this.addMedication} />}
			...
			>
				...
				takeDaily.length === 0 &&
				takeAsNeeded.length === 0 && <div className="ant-list-empty-text">No Medications</div>}
				{takeAtTimes.length > 0 && (
					<div>
						<Divider>
				...
				{this.state.showSummary && (
					<MedicationSummary userId={user_id} date={date} onCancel={this.toggleSummary} />
				)}

				{this.state.addModal && (
					<MedicationManager
						drugId={this.state.medId}
						userId={user_id}
						date={date}
						title={
							<FormattedMessage
								id="plan.medication.add"
								defaultMessage="Add"
				...
				{this.state.showMotivatorsModal && (
					<Modal
						visible={true}
						destroyOnClose
						footer={false}
