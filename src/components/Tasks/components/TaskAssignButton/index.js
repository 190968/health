...
const TaskAssignButtonPure = props => {
    const { showModal, toggleModal, label, asMenuItem, asPlusIcon, icon, asIcon, size='small', title='', asText=false, ...otherProps } = props;
    ...
    return <React.Fragment>
        ...
        {asMenuItem ? <span onClick={toggleModal}>{label}</span> : icon ? <Button icon={icon} size={size} onClick={toggleModal} /> : asText ? <span onClick={toggleModal} >{titleButton}</span> : asPlusIcon ? <Button icon={'plus'} onClick={toggleModal}  size={'small'} /> : <Button onClick={toggleModal} >{titleButton}</Button>}
        </Tooltip>
    </React.Fragment>
}
