...
import {Form, List, Select, Avatar} from 'antd';
import { RideEstimate } from '../../containers/RideEstimate';
...
const productsFake = [
  ...
  const UberProducts = props => {
    const products = productsFake;

    return <Select optionLabelProp="label" style={{width: '100%'}} onChange={props.onChange}>
                {products.map(product => {
                    const {product_id, image, display_name, description} = product;
                    return <Select.Option key={product_id} label={display_name}>
                        {/* <img src={image} style={{height: 14}} />*/} {display_name} 
                        <div style={{fontSize: '0.8em', color: '#ccc'}}>{description}</div>
                     </Select.Option>
                })}
